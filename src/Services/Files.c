...
static void files_real_create (Service* base) {
	...
	self = (Files*) base;
	_tmp0_ = _ ("Files");
	_tmp1_ = g_strdup_printf ("<span font='20' face='Raleway' color='black'>%s</span>", _tmp0_);
	_tmp2_ = _tmp1_;
	_tmp3_ = shadowed_label_new (_tmp2_);
	g_object_ref_sink (_tmp3_);
	_tmp4_ = _tmp3_;
	_g_free0 (_tmp2_);
	files_label = _tmp4_;
	_tmp5_ = shadowed_label_new ("");
	g_object_ref_sink (_tmp5_);
	_g_object_unref0 (self->priv->files_list);
	self->priv->files_list = _tmp5_;
	_tmp6_ = self->priv->files_list;
	_tmp7_ = clutter_actor_get_height ((ClutterActor*) files_label);
	_tmp8_ = _tmp7_;
	clutter_actor_set_y ((ClutterActor*) _tmp6_, _tmp8_ + 12);
	_tmp9_ = self->priv->files_list;
	clutter_actor_set_x ((ClutterActor*) _tmp9_, (gfloat) 12);
	clutter_actor_add_child ((ClutterActor*) self, (ClutterActor*) files_label);
	_tmp10_ = self->priv->files_list;
	clutter_actor_add_child ((ClutterActor*) self, (ClutterActor*) _tmp10_);
	_tmp11_ = self->priv->files_list;
	g_signal_connect_object ((ClutterActor*) _tmp11_, "button-release-event", (GCallback) ___lambda11__clutter_actor_button_release_event, self, 0);
	_tmp12_ = self->priv->settings;
	_tmp13_ = g_settings_get_int (_tmp12_, "update-interval");
	g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) _tmp13_, _service_update_gsource_func, g_object_ref ((Service*) self), g_object_unref);
...
static gboolean files_real_update (Service* base) {
	...
	_tmp1_ = g_strdup ("<span font='12' face='Open Sans' color='black'>");
	res = _tmp1_;
	{
		gint i;
		i = 0;
